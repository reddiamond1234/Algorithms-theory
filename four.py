...
def write_output_to_file(filename, x_coords, y_coords, intersect):
...
merged_hull = jarvis_march(all_points)

# Check if convex hulls intersect
intersect = len(set(merged_hull.flatten())) < len(all_points)

# Write output to the output file
write_output_to_file("output.txt", merged_hull[:, 0], merged_hull[:, 1], intersect)
